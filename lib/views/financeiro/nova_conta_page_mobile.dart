...
class NovaContaPageMobile extends StatefulWidget {
  final String? clienteIdInicial;

  final String? clienteNomeInicial;

  const NovaContaPageMobile({
    super.key,
  ...
  @override
  State<NovaContaPageMobile> createState() => _NovaContaPageMobileState();
}

class _NovaContaPageMobileState extends State<NovaContaPageMobile> {
  final formKey = GlobalKey<FormState>();

  final DividaService service = DividaService();
  ...
  void initState() {
    ...
    clienteId = widget.clienteIdInicial;

    clienteNome = widget.clienteNomeInicial;
  }
  ...
  Future salvar() async {
    ...
    try {
      final total = valorNumero();

      final qtd = int.tryParse(parcelas.text) ?? 1;

      final valorParcela = total / qtd;
      ...
      for (int i = 0; i < qtd; i++) {
        lista.add({
          ...
          "vencimento": Timestamp.fromDate(
            DateTime(vencimento.year, vencimento.month + i, vencimento.day),
          ),
      ...
      await service.criarDivida(divida, lista);
        ...
        children: [
          InkWell(
            ...
            child: Container(
              ...
              child: Icon(Icons.arrow_back_rounded, color: primaria),
            ),
          ),
  ...
  Widget campo(String label, IconData icon, TextEditingController controller) {
    return Padding(
  ...
  Widget campoCliente() {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final clientes = snapshot.data!.docs.map((doc) {
        ...
        return Autocomplete<ClienteModel>(
          ...
          optionsBuilder: (TextEditingValue texto) {
            ...
            final busca = texto.text.toLowerCase();

            return clientes.where((c) {
              final nome = c.nomeExibicao.toLowerCase();

              final telefone = c.telefone.toLowerCase();

              return nome.contains(busca) || telefone.contains(busca);
            });
          },
          ...
          fieldViewBuilder:
              (context, controller, focusNode, onEditingComplete) {
                return TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: clienteNome ?? "Pesquisar cliente",

                    prefixIcon: Icon(Icons.search, color: primaria),
            ...
            child: Form(
              ...
              child: ListView(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: [primaria, const Color(0xFF42A5F5)],
                  ...
                  _titulo("Detalhes da dívida", Icons.receipt_long_rounded),

                  const SizedBox(height: 5),

                  campo("Descrição", Icons.description_outlined, descricao),

                  campo("Categoria", Icons.category_outlined, categoria),

                  campo("Valor total", Icons.attach_money_rounded, valor),
