...
class PropostasClientePage extends StatelessWidget {
  const PropostasClientePage({super.key});

  String get uid => FirebaseAuth.instance.currentUser?.uid ?? '';

  Stream<QuerySnapshot<Map<String, dynamic>>> minhasPropostas() {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          docs.sort((a, b) {
            final dataA = a.data()['dataCriacao'];
            final dataB = b.data()['dataCriacao'];
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final proposta = doc.data();

              final status =
                  proposta['status'] ?? 'aguardando_aprovacao';

              final valor = proposta['valorTotal'] == null
                  ? 0.0
                  : (proposta['valorTotal'] as num).toDouble();
                      ...
                      ElevatedButton.icon(
                        ...
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => VisualizarContratoPage(
                                propostaId: doc.id,
                              ),
                            ),
                          );
